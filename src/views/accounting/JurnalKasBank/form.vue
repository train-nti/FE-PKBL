          ...
          <div v-show="JournalModel.JournalTypeId">
            ...
            <b-row >
                <b-col md="1" class="my-1" v-if="id != null">
                  <router-link :to="{ name: 'Jurnal Kas Bank', params: {} }"><b-col><b-button type="submit" variant="">Kembali</b-button></b-col></router-link>
                </b-col>
  ...
  watch:{
    'JournalModel.JournalTypeId': async function (){
      ...
      if (this.JournalModel.JournalTypeId == 6) {
        ...
        var getall = await apiAccount.getByTypeAndWorkareaISnull(6,this.cabang)
        console.log('pk',getall);
        this.options = [];
        ...
        for (var i = 0; i < this.PaymentType.length; i++) {
          ...
          var getall = await apiAccount.getByTypeAndWorkareaISnull(5,this.cabang)
          console.log('bl',getall);
          this.options = [];
              for (var i = 0; i < getall.length; i++) {
                this.options.push({ code: getall[i].Code, name:getall[i].Code +" - "+ getall[i].Name })
              }
        ...
        var getall = await apiAccount.getByTypeAndWorkareaISnull('5,6',this.cabang)
        console.log('pkbl',getall);
        this.options = [];
        for (var i = 0; i < getall.length; i++) {
          this.options.push({ code: getall[i].Code, name:getall[i].Code +" - "+ getall[i].Name })
        }
