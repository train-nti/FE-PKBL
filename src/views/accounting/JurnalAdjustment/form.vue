        ...
        <form @submit.prevent="SaveJournal">
          ...
          <div v-show="JournalModel.JournalTypeId">
            ...
            <b-row>
              ...
              <b-col>
                <date-picker  style="width: 100%;" v-model="JournalModel.JournalDate"  :disabled="(id != null )&&(status == 'detail')&&(JournalModel.IsPosting != 0) "  :input-attr="{ required: true }" valueType="YYYY-MM-DD" format="DD/MMM/YYYY" lang="id"></date-picker>
                <!-- <b-input class="mb-2 mb-sm-0" :disabled="(id != null )&&(JournalModel.IsPosting != 0) " v-model="JournalModel.JournalDate" type="date" id="#" placeholder="" required readonly/> -->
              </b-col>
            </b-row>
            <br>
            ...
            <b-row >
                <b-col md="1" class="my-1" v-if="id != null">
                  <router-link :to="{ name: 'Jurnal Adjustment', params: {} }"><b-col><b-button type="submit" variant="">Kembali</b-button></b-col></router-link>
                </b-col>
...
export default {
  ...
  async created(){
    ...
    // let generateCode = this.cabang + '.2019.AD.IX.' + counter
    // this.JournalModel.JournalNumber = generateCode

    this.dataTableJournal.Search = this.cabang + '.'+  new Date().getFullYear() +'.AD.'+ this.formatRomawi(new Date().getMonth() + 1) +'.'
    ...
    var akhirtahun = moment().endOf('year').format('YYYY-MM-DD');
    // console.log('enddate',akhirtahun);
    this.JournalModel.JournalDate = akhirtahun
    console.log('date',akhirtahun);
    ...
    // console.log(this.options_PaymentType);
  ...
  watch:{
    'JournalModel.JournalTypeId': async function (){
      this.options_PaymentType = []
      if (this.JournalModel.JournalTypeId == 18) {
      ...
      }else if (this.JournalModel.JournalTypeId == 19) {
        for (var i = 0; i < this.PaymentType.length; i++) {
  ...
  methods: {
          ...
          .then(async (value) => {
                if(value){
                    ...
                    if (isUpdate) {
                      ...
                      this.$router.push('/accounting/jurnal-adjustment')
    ...
    // formatEndDate(value){
    //   return moment(String(value)).endOf("year");
    // },
    showModal () {
