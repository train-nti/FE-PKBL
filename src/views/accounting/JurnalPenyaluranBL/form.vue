...
<div class="animated fadeIn">
  ...
  <b-row v-if="!loading">
    <b-col sm="12">
      <b-card>
        <div slot="header">
          <strong>Jurnal Penyaluran BL</strong>
        </div>
        <form @submit.prevent="SaveJournal">
          ...
          <b-row >
              <b-col md="1" class="my-1" v-if="id != null">
                <router-link :to="{ name: 'Jurnal Penyaluran BL', params: {} }"><b-col><b-button type="submit" variant="danger">Kembali</b-button></b-col></router-link>
              </b-col>
...
export default {
  ...
  async created(){
    ...
    // let generateCode = '2019.PCR.KB.' + counter
    // this.JournalModel.JournalNumber = generateCode

    this.dataTableJournal.Search = new Date().getFullYear() +'.PCR.'+ 'KB' +'.'
    ...
    console.log("apake",this.PaymentType);
    // this.loading = true
 ...
 //       var getall = await apiAccount.getByType(1)
 ...
 //       }
 //     }else {
 //       var getall1 = await apiAccount.getNoType(1)
      ...
      if (isUpdate) {
        ...
        // await apiJournalItem.update(this.$route.params.id, this.items);
          ...
          if (this.items.length > 0) {
            ...
            this.$router.push('/accounting/jurnal-penyaluran-bl')
