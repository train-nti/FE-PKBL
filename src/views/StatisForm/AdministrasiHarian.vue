...
<div class="">
  <div class="card">
    <div class="card-body">
    	 <b-button class="mt-3" variant="primary" block @click="showModal">Closing Harian</b-button>
	...
	async mounted() {
	  ...
	  methods: {
		 ...
		 async simpanData(){
		  	...
		  	var ubahConfiq = await apiConfiq.update("POSTING_DAILY",this.Confiq)
		  	var ubahClosingDaily = await apiRunnable.getSPClosingDaily(this.Administrasi)
		  	console.log("UbahClosing",ubahClosingDaily)
		  	this.hideModal()
		  	 this.showConfiq()
		  },
		 async showConfiq(){
		  	var dataConfiq = await apiConfiq.getByCode("POSTING_DAILY")
