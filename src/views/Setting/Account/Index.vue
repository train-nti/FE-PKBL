...
<div class="">
  <CrudTable :isBusy="isBusy" :items="items" :fields="fields">
    ...
    <template slot="buttonShowAndEdit" slot-scope="{ row }">
      <b-button pill variant="primary" @click="modalInfo(row)" size="md" v-b-tooltip.hover title="Lihat Detail Account">
        <i class="fa fa fa-eye"></i>
      </b-button>
      <b-button pill variant="success" @click="modalEdit(row.Code)" size="md" v-b-tooltip.hover title="Ubah Data Account">
        <i class="fa fa-pencil-square-o"></i>
      </b-button>
      <b-button pill variant="danger" @click="modalDelete(row.Code)" size="md" v-b-tooltip.hover title="Hapus Data Account">
  ...
  <b-modal :id="infoModal.id" :title="infoModal.title" :size="infoModal.size" hide-footer ok-only>
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-10">
        <v-select :options="dropdownAcGrp" v-model="dataModelInfo.GrpCode" :disabled="true" :reduce="Name => Name.Code" label="Name" />
      </div>
    </div>
  </b-modal>
  ...
  <b-modal :id="editModal.id" :title="editModal.title" :size="editModal.size" hide-footer ok-only>
    <b-form @submit="update">
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <v-select :options="dropdownAcGrp" v-model="dataModelEdit.GrpCode" :reduce="Name => Name.Code" label="Name" />
        </div>
      </div>
  ...
  <b-modal :id="createModal.id" :title="createModal.title" :size="createModal.size" hide-footer ok-only>
    <b-form @submit="save" @reset="onReset">
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10">
          <v-select :options="dropdownAcGrp" v-model="dataModelCreate.GrpCode" :reduce="Name => Name.Code" label="Name" />
        </div>
      </div>
...
export default {
  ...
  methods: {
    async fetch() {
      this.isBusy = true
      let data = await apiAccount.getAll()
    ...
    async modalEdit(id) {
      ...
      this.Code = id.replace(/\s/g, '');
      let data = await apiAccount.getOne(this.Code)
    ...
    async save(evt) {
       ...
       .then(async (value) => {
          if(value){
            ...
            let data = await apiAccount.create(this.dataModelCreate)
            this.saveBtn = true
            this.fetch()
    ...
    async update(evt) {
       ...
       .then(async (value) => {
          if(value){
            ...
            let data = await apiAccount.updatePerId(this.Code, this.dataModelEdit)
            this.updateBtn = true
            this.fetch()
            this.showNotif(data, 'Account')
    ...
    async softDelete(id){
      ...
      let data = await apiAccount.delete(id)
      this.fetch()
      this.showNotif(data, 'Account')
