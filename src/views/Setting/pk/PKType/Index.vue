<template>
<div class="">
  <CrudTable label="Jenis Pinjaman" :isBusy="isBusy" :items="items" :fields="fields">
    ...
    <template slot="buttonShowAndEdit" slot-scope="{ row }">
      <b-button pill variant="primary" @click="modalInfo(row)" size="md" v-b-tooltip.hover title="Lihat Detail Jenis Pinjaman">
        <i class="fa fa fa-eye"></i>
      </b-button>
      <b-button pill variant="primary" @click="modalEdit(row.PkTypeId)" size="md" v-b-tooltip.hover title="Ubah Data Jenis Pinjaman">
...
export default {
  ...
  methods: {
    async fetch() {
      this.isBusy = true
      let data = await apiPKType.getAll()
    ...
    async modalEdit(id) {
      ...
      let data = await apiPKType.getOne(id)
    ...
    async save(evt) {
       ...
       .then(async (value) => {
          if(value){
              ...
              let data = await apiPKType.create(this.dataModelCreate)
              this.saveBtn = true
              this.fetch()
              this.showNotif(data, 'Jenis Pinjaman')
          }
       })
       .catch(err => {
        // An error occurred
    ...
    async update(evt) {
       ...
       .then(async (value) => {
          if(value){
              ...
              let data = await apiPKType.update(this.dataModelEdit.PkTypeId, this.dataModelEdit)
              this.updateBtn = true
              this.fetch()
              this.showNotif(data, 'Jenis Pinjaman')
